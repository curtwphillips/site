...
import { SortableTodo } from './notesSortableTodo';
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
  ...
  return (
    <div className="category-container">
      ...
      <FontAwesomeIcon
        ...
        icon={faTrash}
        onClick={() => setShowCategoryAlert(!showCategoryAlert)}
      />

      <Alert show={showCategoryAlert} variant="warning">
        <Alert.Heading>Are you sure?</Alert.Heading>
        <p>{`This will delete '${category}' and any todos it included.`}</p>
        <hr />
      ...
      <FontAwesomeIcon
        key={category + '-hide-icon'}
        size={'1x'}
        className="inline lower-icon"
        icon={data[index].hidden ? faMinus : faAngleDown}
        onClick={() => hideCategory(category)}
      />
      <div key={category + '-outer-div'}>
        {/* container for hiding todos in category */}
        <div
          key={category + '-category-hide-div'}
          className={data[index].hidden ? 'hidden' : ''}
        >
          {/* check/uncheck all */}
          <div className="todo-row inline">
            <Form.Group className="mb-3 inline" controlId="todoList">
              <Form.Check
                onChange={(e) => updateAllChecked(e, category, setAllChecked)}
                className="inline ml-10"
                type="checkbox"
                checked={allChecked}
              />
          ...
          >
            <SortableContext
              key={`sortable-context-${category}`}
              items={data[index].notes.map((note) => note.order.toString())}
              strategy={verticalListSortingStrategy}
            >
              {data[index].notes.map((note, noteIndex) => (
                <SortableTodo
                  handle
                  categoryIndex={index}
                  todoIndex={noteIndex}
          ...
          <div key={category + '-add'} className="todo-row mt-3 mb-5">
            <Form.Group className="mb-3" controlId="todoInput">
              <Form.Control
                ref={data[index].todoTextRef}
                onKeyPress={(e) => onTodoKeyUp(e, category)}
              ...
              <FontAwesomeIcon
                className="inline"
                icon={faPlus}
                onClick={() => addTodo(category, null, data[index].todoTextRef)}
              />
            </Form.Group>
          </div>
        </div>
